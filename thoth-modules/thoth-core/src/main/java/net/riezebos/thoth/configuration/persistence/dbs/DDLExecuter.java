...
import net.riezebos.thoth.util.OutputListener;
...
public class DDLExecuter {
  ...
  private final List<TranslationPair> _translations = new ArrayList<TranslationPair>();
  private final List<TranslationPair> _workarounds = new ArrayList<TranslationPair>();
  private final List<OutputListener> _listeners = new ArrayList<OutputListener>();
  ...
  protected String applyDialect(String command) {
    ...
    for (TranslationPair wa : _translations) {
      if (!wa.exclude(command)) {
        command = ThothUtil.replaceWord(command + " ", wa.getFrom(), wa.getTo(), SEPARATORS).trim();
      }
    }

    for (TranslationPair wa : _workarounds) {
  ...
  private void initDialect(DatabaseIdiom idiom) {
    ...
    try {
      ...
      databaseIdiom.initDialect(_translations, _workarounds);
  ...
  protected void addWorkaround(String from, String to) {
    _workarounds.add(new TranslationPair(from, to));
  }

  protected void addTranslation(String name, String translatedType) {
    if (translatedType != null) {
      // If there is a precision specified as a parameter i.e. decimal->number($p,$s) then strip the ($p,$s) part
      ...
      if (!translatedType.equalsIgnoreCase(name)) {
        ...
        _translations.add(new TranslationPair(name, translatedType));
      }
    } else {
  ...
  public void println(String msg) {
    LOG.debug(msg);
    for (OutputListener lsnr : _listeners)
      lsnr.println(msg);
  }
